...
namespace SOV.Social
{
    public partial class UCAddrs : UserControl
    {
        ...
        public void Fill(int? addrTypeIdIn, int? addrTypeIdNotIn, int? selectedAddrId)
        {
            AddrTypeIdIn = addrTypeIdIn;
            AddrTypeIdNotIn = addrTypeIdNotIn;
        ...
        {
            tv.Nodes.Clear();
            List<Addr> Addrs = DataManager.GetInstance().AddrRepository
                .Select()
                .Where(x => (!AddrTypeIdIn.HasValue || x.TypeId == AddrTypeIdIn) && (!AddrTypeIdNotIn.HasValue || x.TypeId != AddrTypeIdNotIn))
                .OrderBy(x => x.Name)
                .ToList();

            // FILL
            List<Addr> parents = Addrs.FindAll(x => x.ParentId == null).OrderBy(x => x.Name).ToList();
            Addr.FillChilds(parents, Addrs);
            foreach (Addr arParent in parents)
            {
