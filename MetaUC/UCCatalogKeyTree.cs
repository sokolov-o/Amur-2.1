...
{
    /// <summary>
    /// Дерево ключей каталога данных - для установки фильтра и др. нужд.
    /// </summary>
    public partial class UCCatalogKeyTree : UserControl
    {
        public UCCatalogKeyTree()
        {
            InitializeComponent();
        }
        public UCCatalogKeyTree(string connectionString)
        {
            InitializeComponent();
        }
        ...
        public void Fill()
        {
            ...
            foreach (var item in sg.OrderBy(x => x.Name))
            {
                ...
                {
                    TreeNode node2 = node1.Nodes.Add("u", site.NameWithCodeRight);
                    node2.Tag = site.Id;
            ...
            foreach (var item in eg.OrderBy(x => x.Key.Name))
            {
                ...
                foreach (var varId in item.Value)
                {
                    ...
                    TreeNode node2 = node1.Nodes.Add("u", var.Name);
                    node2.Tag = var.Id;
                    node2.ImageIndex = node2.SelectedImageIndex = 1;
                }
            }
            ...
            if (eg.Count > 0)
            {
                List<Source> sources = Meta.DataManager.GetInstance().SourceRepository.Select();
                ...
                foreach (var item1 in eg.OrderBy(x => x.Key.Name))
                {
                    ...
                    foreach (var item2 in sources.Where(x => item1.Value.Exists(y => y[0] == x.Id)).OrderBy(x => x.Name))
                    {
                        TreeNode node2 = node1.Nodes.Add("u", item2.Name);
                        node2.Tag = item2.Id;
                        node2.ImageIndex = node2.SelectedImageIndex = 1;
